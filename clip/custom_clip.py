...
# MTA 专属
class ClipTestTimeTuning_mta(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
                 n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False):
        super(ClipTestTimeTuning_mta, self).__init__()
        clip, _, _ = load(arch, device=device, download_root=DOWNLOAD_ROOT)
        self.image_encoder = clip.visual
        self.text_encoder = TextEncoder(clip)
        self.logit_scale = clip.logit_scale.data
...
def get_coop(method, clip_arch, test_set, device, n_ctx, ctx_init, learned_cls=False):
    ...
    if method == 'mta':
        model = ClipTestTimeTuning_mta(device, classnames, None, arch=clip_arch,
