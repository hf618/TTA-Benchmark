...
# MTA 元素
from ensemble.MTA.mta import solve_mta
# C-TPT 元素
from ensemble.C_TPT.funcs import ECE_Loss, Calculator
...
def main_worker(gpu, args, result_dict):
    ...
    if args.cocoop:
        ...
        model = get_coop(method, args.arch, args.test_sets, args.gpu, args.n_ctx, args.ctx_init)
    ...
    for set_id in datasets:
        if args.tpt or args.mta:
            ...
            if args.ctpt and args.I_augmix:
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, result_dict):
    ...
    if args.ctpt and ('ctpt' in args.run_type):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ctpt', action='store_true', default=False, help='run ctpt')
    ...
    parser.add_argument('--lambda_q', type=float, default=4, help='quadratic term weighting factor')
    parser.add_argument('--lambda_y', type=float, default=0.2, help='entropic term weighting factor')
