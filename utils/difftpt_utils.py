...
class diffuData(Dataset):
    def __init__(self, data_root, img_list, label_list, trainsform, augmode='difftpt', diffu_ratio=0.5, view_num=63):
        self.data_root = data_root # 包含图像数据的根目录的路径。
        self.img_list = img_list  # 一个包含图像文件路径的列表。
        self.label_list = label_list # 对应于 img_list 中每个图像的标签列表。
        # 一个包含转换操作的列表，用于对图像进行预处理或数据增强。
        # 根据 augmode 的不同，可能会使用列表中的不同转换。
    ...
    def __getitem__(self, item):
        if self.augmode == 'tpt':
            # 加载 img_list 中的图像，并将其转换为 RGB 格式。
            # 应用 trainsform[0] 转换，并返回转换后的图像和对应的标签。
            ...
            # diffu 64 + aug 64
            # 加载原始图像，并应用 trainsform[0] 转换。
            img = Image.open(self.img_list[item]).convert('RGB')
            imgs = self.trainsform[0](img) # imgs[ori，augx63]

            diffu_img = [] # 63 diffu imgs
            # 对于 view_num 指定的视图数量，加载每个视图的图像，
            # 这些图像可能是通过某种方式（如旋转、缩放等）与原始图像不同的变体。
...
def get_data_loader(data_transform, args=None):
    ...
    img_list = []
    label_list = []
    print("args.diff_root",args.diff_root)
